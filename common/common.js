...
const getOfferNameBy_ID = async (_id) => {
    let offerName = await Offer.findOne({ _id: _id });
    if (offerName) {
        return { "name": offerName.offer_name };
    } else {
        return "";
    }
}
...
const getPublisherDataByPubId = async (pub_id) => {
    let publisher = await Publisher.findOne({ 'pub_id': parseInt(pub_id) });
    if (publisher) {
        return publisher;
    } else {
        return "";
