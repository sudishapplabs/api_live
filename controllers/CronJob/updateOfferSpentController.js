...
exports.getOfferSpent = async (req, res) => {
  ...
  if (Array.isArray(offers) && offers.length > 0) {
      ...
      for (let p = 0; p < dateArrayData[offerIdOverSixMonth].length - 1; p += 1) {
        if (typeof dateArrayData[offerIdOverSixMonth][p + 1]) {
          ...
          if (typeof dateArrayData[offerIdOverSixMonth][p + 1] !== 'undefined' && dateArrayData[offerIdOverSixMonth][p + 1] !== "") {
            var endDate_s = dateArrayData[offerIdOverSixMonth][p + 1];
          }
          const endpoint = "reports/custom"
          // GET DATA FROm TRACKIER
          await axios.get(process.env.API_BASE_URL + endpoint + "?group[]=campaign_name&group[]=campaign_id&kpi[]=grossClicks&kpi[]=grossRevenue&kpi[]=grossConversions&" + offer_str_id + "start=" + fromDate + "&end=" + endDate_s + "&zone=Asia/Kolkata", axios_header).then((totSixMonthRes) => {
    ...
    const response = { 'success': true, 'message': 'Offer spent DB updated!!' };
