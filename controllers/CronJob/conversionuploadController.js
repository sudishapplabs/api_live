...
exports.uploadConversions = async (req, res) => {
  ...
  if (Array.isArray(cnvData) && cnvData.length > 0) {
    ...
    for (let i = 0; i < cnvData.length; i++) {
      ...
      Conversion.findByIdAndUpdate({ _id }, { '$set': convProcessing }, { new: true }).exec().then((resCnv) => {
        if (!resCnv) {
    ...
    axios.post(process.env.API_BASE_URL + "conversions/bulkUpload", conversionData, axios_header).then((resCnv) => {
      if (typeof resCnv.data.success !== 'undefined' && resCnv.data.success == true) {
        for (let j = 0; j < cnvData.length; j++) {
          ...
          Conversion.findByIdAndUpdate({ _id: cnvs._id }, { '$set': conversionJobData }).exec().then((convJob) => {
            if (!convJob) {
              const resMsg = { "success": false, "message": "Something went wrong please try again!!" };
              res.status(200).send(resMsg);
              return;
