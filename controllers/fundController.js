...
exports.addFund = async (req, res) => {
  ...
  try {
    const create_payment_json = {
      ...
      "redirect_urls": {
        "return_url": "https://api-stage.applabs.ai/stage/fund/success?tid=" + tid + "&amount=" + amount + "&added_by_name=" + encodeURI(added_by_name) + "&added_by_email=" + added_by_email,
        "cancel_url": "https://api-stage.applabs.ai/stage/fund/cancel?tid=" + tid + "&amount=" + amount + "&added_by_name=" + encodeURI(added_by_name) + "&added_by_email=" + added_by_email
      },
      "transactions": [{
        "item_list": {
          "items": [{
        ...
        if (updateBalance) {
          ...
          if (typeof action !== 'undefined' && action == 'debited') {
            ...
            const notificationData = {
              ...
              subject_adv: 'Account ' + ucwords(updateBalance.organization) + ' Balance Low to run Campaign',
            ...
            if (updateBalance.email_preferences == true) {
              ...
              const msg_user = {
                ...
                bcc: bcc_mail,
                subject: 'Applabs Alert - Account ' + ucwords(updateBalance.organization) + ' Balance Low to run Campaign',
                html: messageBodyUser
              };
              //ES6
              sgMail.send(msg_user).then(() => { }, error => {
